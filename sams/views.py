...
def vendor(request):
    vendor_list= Vendor.objects.all() #retornar una lista de vendedores
    #F= se nombra primero el data_context después la variable creada
...
def product(request):#filter te trae una lista de objetos que cumplen con una restricción
    product_list= Product.objects.filter(deleted_date__isnull = True) #retornar una lista de vendedores
    #F= se nombra primero el data_context después la variable creada
...
def deleteproduct(request,product_id):
    ...
    #1° nombre del valor, 2° Nombre de la variable
